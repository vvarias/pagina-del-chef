let recetas = [
    ...
    },
    {
        ...
        ingredients: ["1 kg de judías verdes", "200 g de almendras tostadas y fileteadas", "2 cabezas de ajo", "2 cucharadas de mostaza de Dijon", "2 cucharadas de miel", "Aceite de oliva","Sal", "Pimienta"],
...
function buscar() {
    const palabraClave = document.getElementById('name').value.toLowerCase();
    let nombreReceta;
    let ingredients;
    let preparation;
    ...
    for (let i = 0; i < recetas.length; i++) {
        ...
        if (receta.name.toLowerCase().includes(palabraClave) || receta.ingredients.some(ingredient => ingredient.toLowerCase().includes(palabraClave))) {
            nombreReceta = receta.name;
    ...
    if (nombreReceta) {
        ...
        document.getElementById('ingredients').innerText = "Ingredientes: " + ingredients.join(", ");
        //mostrando los pasos de preparación
        const lista = document.getElementById('recetas');
        lista.innerHTML = ""; // Limpiamos la lista anterior
        const ul = document.createElement('ul');
        const pasos = preparation.split('. ');
        for (let i = 0; i < pasos.length; i++) {
            const li = document.createElement('li');
            li.textContent = pasos[i];
            ul.appendChild(li);
        }
        lista.appendChild(ul);
    } else {
        document.getElementById('nombreReceta').innerText = "Receta no encontrada";
